...
/// Walk through all pictures in test/fixtures/pictures, run exiftool on each,
/// and compare the output with the library's parsing
pub fn auto_json_comparison_test() {
  let pictures_dir = "test/fixtures/pictures"

  // Get all files from the pictures directory using simplifile
  let assert Ok(files) = simplifile.read_directory(pictures_dir)

  // Filter for image files and build full paths
  let picture_paths =
    files
    |> list.filter(fn(file) {
  ...
  list.each(picture_paths, fn(pic_path) {
    ...
    // Create snapshot of library output
    library_parsed |> pprint.format |> birdie.snap(title: pic_path)

    // Compare exiftool JSON output with library output
    json_parsed
    |> should.be_ok
    |> list.first
    |> should.be_ok
